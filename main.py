...
sepalValue = st.sidebar.slider('sepal_length_cm', min_value=0.0, max_value=10.0, step=0.1)
petalValue = st.sidebar.slider('petal_length_cm', min_value=0.0, max_value=10.0, step=0.1)
...
record = pd.Series([sepalValue, petalValue], index=value_df.columns, name='data')
value_df = pd.concat([value_df, record.to_frame().T])
